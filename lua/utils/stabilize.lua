...
local cfg = {
	force = true,
	forcemark = nil,
	ignore = { filetype = { "help", "list", "Trouble" }, buftype = { "terminal", "quickfix", "loclist" } },
}
local windows = {}
...
api.nvim_create_autocmd(
	"WinClosed",
	{ group = group, callback = function() M.handle_closed(tonumber(vim.fn.expand "<afile>")) end }
)
api.nvim_create_autocmd(
	{ "BufWinEnter", "CursorMoved", "CursorMovedI" },
	{ group = group, callback = function() M.save_window() end }
)
api.nvim_create_autocmd(
	"User",
	{ group = group, pattern = "StabilizeRestore", callback = function() M.restore_windows() end }
)

if cfg.nested then
	api.nvim_create_autocmd(
		cfg.nested,
		{ group = group, callback = function() api.nvim_exec_autocmd("User", { pattern = "StabilizeRestore" }) end }
	)
end
