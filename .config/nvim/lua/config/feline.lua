...
options.nvim_gps = {
	provider = function()
		local lazy, gps = pcall(require, "nvim-gps")
		if not lazy then
			return ""
		end
...
-- left
add_table(options.left, options.empty_space)
add_table(options.left, options.empty_spaceColored)
add_table(options.left, options.mode_icon)
add_table(options.left, options.mode_name)
...
add_table(options.left, options.nvim_gps)
