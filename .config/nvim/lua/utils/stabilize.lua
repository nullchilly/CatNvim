-- Upstream: https://github.com/luukvbaal/stabilize.nvim
-- Core pr: https://github.com/neovim/neovim/pull/19243
-- About: stabilize window open/close events
local M = {}
local api = vim.api
local fn = vim.fn
...
local function add_win(win)
	if
		windows[win]
		or vim.tbl_contains(cfg.ignore.filetype, api.nvim_buf_get_option(0, "filetype"))
		or vim.tbl_contains(cfg.ignore.buftype, api.nvim_buf_get_option(0, "buftype"))
		or vim.F.npcall(api.nvim_win_get_var, 0, "previewwindow")
	then
		return
	end
	windows[win] = {
...
api.nvim_create_autocmd("WinNew", { group = group, callback = function() M.handle_new() end })
api.nvim_create_autocmd("WinClosed", { group = group, callback = function() M.handle_closed(tonumber(vim.fn.expand "<afile>")) end })
api.nvim_create_autocmd({ "BufWinEnter", "CursorMoved", "CursorMovedI" }, { group = group, callback = function() M.save_window() end })
api.nvim_create_autocmd("User", { group = group, pattern = "StabilizeRestore", callback = function() M.restore_windows() end })

if cfg.nested then api.nvim_create_autocmd(cfg.nested, { group = group, callback = function() api.nvim_exec_autocmd("User", { pattern = "StabilizeRestore" }) end }) end
